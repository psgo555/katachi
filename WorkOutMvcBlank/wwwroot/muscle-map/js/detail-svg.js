...
const DETAIL_SVG_CONFIGS = {
  ...
  calves: {
    ...
    svgSrc: '/muscle-map/svg/detail/calvesBack.svg'
  },
}
...
function renderDetailSubExercises(container, muscleName, regionKey, regionLabel) {
  ...
  if (region.exercises.length === 0) {
    ...
    region.exercises.forEach(ex => {
      ...
      nameLink.href = `/MuscleMap/Exercise?muscle=${muscleName}&equipment=${targetEquipment}&name=${encodeURIComponent(ex.name)}`
