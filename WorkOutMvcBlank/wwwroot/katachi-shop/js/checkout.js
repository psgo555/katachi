...
shipOptions.forEach(option => {
  option.addEventListener('click', () => {
    shipOptions.forEach(o => o.classList.remove('is-selected'));
    option.classList.add('is-selected');
    selectedShipping = option.querySelector('input').value;
    renderOrder();
  });
});
...
function autoSelectFreeShipping(subtotal) {
  if (subtotal >= 2000) {
    shipOptions.forEach(o => o.classList.remove('is-selected'));
    document.getElementById('shipFree').classList.add('is-selected');
...
function validateForm() {
  const required = ['lastName', 'firstName', 'phone', 'email', 'address', 'city'];
